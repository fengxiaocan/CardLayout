...
class CardViewBaseImpl implements CardViewImpl {
		...
		RoundRectDrawableWithShadow.sRoundRectHelper = new RoundRectDrawableWithShadow.RoundRectHelper() {
			public void drawRoundRect(Canvas canvas,RectF bounds,float cornerRadius,Paint paint) {
				...
				if (cornerRadius >= 1.0F) {
					...
					CardViewBaseImpl.this.mCornerRect
			...
			float elevation,float maxElevation)
	{
		RoundRectDrawableWithShadow background = this
				.createBackground(context,backgroundColor,radius,elevation,maxElevation);
	...
	private RoundRectDrawableWithShadow createBackground(
			Context context,ColorStateList backgroundColor,float radius,float elevation,
			float maxElevation)
	{

 ...
 * @desc ...
 */
public class CardImageButton extends ImageButton implements ICardLayout {
    private final CardLayoutHelper HELPER;

    public CardImageButton(@NonNull Context context) {
        this(context, (AttributeSet) null);
    }

    public CardImageButton(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, android.support.v7.cardview.R.attr.cardViewStyle);
    }

    public CardImageButton(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr)
    {
        super(context, attrs, defStyleAttr);
        HELPER = new CardLayoutHelper(new ICardView() {
            @Override
            public View getCurrentView() {
                return CardImageButton.this;
            }

            @SuppressLint("WrongCall")
            @Override
            public void superOnMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                CardImageButton.super.onMeasure(widthMeasureSpec, heightMeasureSpec);
            }
